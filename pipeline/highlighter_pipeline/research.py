...
as `research_context` (llm.py threads it into the scoring prompt).

This will grow into a LangGraph multi-agent workflow later; for now it is
deliberately one call to keep costs down.
"""
...
def research_content_context(
    *,
    ...
    """Run one web-grounded research call and return a dict matching
