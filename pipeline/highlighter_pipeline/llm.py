...
difficulties, and meandering that goes nowhere. Selected segments should be
self-contained thoughts — typically from ~30 seconds up to several minutes —
and a chunk may contribute zero, one, or several segments.
...
material that would produce an edit of that scale — not as a quota to fill.
...
chunks are edited separately and overlapping or adjacent selections are merged,
so a passage larger than one chunk still comes together in the final cut.
...
def system_prompt(pipeline_mode: str, *, target_length: str | None = None) -> str:
    """The editor system prompt for a pipeline mode ('short' or 'long')."""
    if pipeline_mode == "long":
        return LONGFORM_SYSTEM_PROMPT.format(
            target_length=target_length or DEFAULT_TARGET_LENGTH_MINUTES
        )
...
def detect_clip_candidates(
    *,
    ...
    decision = asyncio.run(
        _detect_clip_candidates_openrouter(
...
async def _detect_clip_candidates_openrouter(
    *,
    transcript: str,
    words: list[dict[str, Any]],
    chunk_index: int,
    ...
    try:
        response = await client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": system_prompt(pipeline_mode, target_length=target_length),
                },
